...
MOCK_FILE = os.path.join(
    os.path.dirname(__file__), "../mocks/artifactory/artifactory_content.json"
)
...
def download_binary(repo: str, full_path: str):
    if repo not in MOCK_BINARIES:
        return JSONResponse(
            status_code=404, content={"error": f"Repo '{repo}' not found"}
        )

    if full_path not in MOCK_BINARIES[repo]:
        return JSONResponse(
            status_code=404, content={"error": f"Binary '{full_path}' not found"}
        )

    return JSONResponse(
        status_code=200,
        content={"message": "Binary exists", "repo": repo, "path": full_path},
    )
...
async def upload_binary(repo: str, full_path: str):
    ...
    return JSONResponse(
        status_code=200,
        content={"message": "Binary already exists", "repo": repo, "path": full_path},
    )
...
async def check_repo_exists(repo: str):
    ...
    return JSONResponse(
        status_code=404, content={"error": f"Repository '{repo}' not found."}
    )
